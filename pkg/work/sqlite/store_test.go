...
func TestRetryableFailureReturnsJobToQueue(t *testing.T) {
	...
	if err := store.FailJob(ctx, leased.JobID, "worker-a", "temporary timeout", true, 0); err != nil {
...
func TestNonRetryableFailureMarksJobFailed(t *testing.T) {
	...
	if err := store.FailJob(ctx, leased.JobID, "worker-a", "contract failed", false, 0); err != nil {
...
func TestWrongWorkerCannotFailJob(t *testing.T) {
	...
	err = store.FailJob(ctx, leased.JobID, "worker-b", "wrong owner", false, 0)
	if err != work.ErrNotFound {
